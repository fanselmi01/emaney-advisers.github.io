...
/* Blog - Ver mas */
document.addEventListener("DOMContentLoaded", function () {
    var verMasButton = document.getElementById("ver-mas");
    var articulosOcultos = document.querySelectorAll(".card-blog.oculto");
    var maxVisible = 3;

    verMasButton.addEventListener("click", function (e) {
        e.preventDefault();

        articulosOcultos.forEach(function (articulo, index) {
            if (index < maxVisible) {
                articulo.classList.toggle("oculto");
            }
        });

        verMasButton.textContent = articulosOcultos[0].classList.contains("oculto") ? "Ver más" : "Ver menos";
    });
});
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            tabs.forEach(function (tab) {
                tab.addEventListener('change', function () {
                    // Desactivar todos los demás elementos
